...
export const ActiveSessionDisplay: React.FC<{
    ...
    const triggeredAlarmsRef = useRef(new Set<string>());
    ...
    const showNotification = useCallback((body: string) => {
        if ('Notification' in window && Notification.permission === 'granted') {
            new Notification(`Screen Time Reminder for ${activity.name}`, {
                body,
                icon: '/vite.svg',
                badge: '/vite.svg',
            });
    ...
    const playAlarm = useCallback(async (text: string) => {
    ...
    useEffect(() => {
        // Clear previously triggered alarms for this new session
        triggeredAlarmsRef.current.clear();
        requestNotificationPermission();

        const checkAlarms = (currentElapsedTime: number) => {
            const elapsedMinutes = Math.floor(currentElapsedTime / 60000);
            
            const triggerAlarm = (key: string, message: string) => {
            ...
            if (todayAllotment > 0) {
                ...
                if (remainingMinutes <= 60 && remainingMinutes > 59) triggerAlarm('rem-60', 'you have 1 hour left!');
                else if (remainingMinutes <= 30 && remainingMinutes > 29) triggerAlarm('rem-30', 'you have 30 minutes left!');
                else if (remainingMinutes <= 10 && remainingMinutes > 9) triggerAlarm('rem-10', 'just 10 minutes left!');
                else if (remainingMs <= 0 && !triggeredAlarmsRef.current.has('rem-0')) triggerAlarm('rem-0', 'your screen time is up!');
            } else {
                if (elapsedMinutes >= 30 && elapsedMinutes < 31) triggerAlarm('dur-30', 'you have been on screen for 30 minutes!');
                else if (elapsedMinutes >= 45 && elapsedMinutes < 46) triggerAlarm('dur-45', 'you have been on screen for 45 minutes!');
                else if (elapsedMinutes >= 60 && elapsedMinutes < 61) triggerAlarm('dur-60', 'you have been on screen for 1 hour!');
                else if (elapsedMinutes >= 120 && elapsedMinutes < 121) triggerAlarm('dur-120', 'you have been on screen for 2 hours!');
                else if (elapsedMinutes >= 180 && elapsedMinutes < 181) triggerAlarm('dur-180', 'you have been on screen for 3 hours!');
            }
        };
        ...
        const intervalId = setInterval(() => {
            ...
            checkAlarms(currentElapsedTime);
        }, 1000);
    ...
    }, [startTime, totalTimeToday, todayAllotment, playAlarm, showNotification, requestNotificationPermission]);
